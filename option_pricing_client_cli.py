...
def run_client(
    ...
    if option_type_str.upper() == "CALL":
        ...
        raise ValueError(
            f"Invalid option type: {option_type_str}. Must be 'CALL' or 'PUT'."
        )
    ...
    with grpc.insecure_channel(server_address) as channel:
        ...
        try:
            ...
            print(
                f"Option Type: {'CALL' if response.option_type == black_scholes_pb2.CALL else 'PUT'}"
            )
...
def main():
    ...
    parser.add_argument("--server", help="Server address in format host:port")
    parser.add_argument(
        "--stock-price", type=float, required=True, help="Current stock price"
    )
    parser.add_argument(
        "--strike-price", type=float, required=True, help="Strike price"
    )
    parser.add_argument(
        "--risk-free-rate",
        type=float,
